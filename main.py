...
TOKEN = "8746793457:AAGCDAU_YpiUrO78ILcu2MNCXQiU0cnpVYk"
DATABASE = '/home/ubuntu/runbot/payments.db'
PROVIDER_TOKEN = '187703658:TEST:5d5b04968f5d1a03e9fc853d6895cf8f8f5254fb'
ADMIN_IDS = [7972155518]
NOTIFY_IDS = [7972155518]

# v2: 2-bot payment architecture
PAYMENT_BOT_USERNAME = 'shyareeet_bot'
...
def init_db():
    ...
    with sqlite3.connect(DATABASE) as conn:
        ...
        cursor.execute('''CREATE TABLE IF NOT EXISTS pending_deliveries (
...
# v2: token-based delivery helper and HTTP API for payment bot handoff
def _deliver_pending_token(token, chat_id=None):
    """Look up token in pending_deliveries and enqueue delivery.
    Returns (ok, msg, code) where code in {delivered, already, missing, no_videos, error}.
    """
    try:
        with sqlite3.connect(DATABASE) as conn:
            row = conn.execute(
                'SELECT user_id, video_count, delivered FROM pending_deliveries WHERE token = ?',
                (token,)
            ).fetchone()
        if not row:
            return False, "Unknown or expired delivery token.", "missing"
        user_id, count, delivered = row
        if delivered:
            return False, "Already processed.", "already"
        unsent = get_unsent_videos(user_id, limit=count)
        if not unsent:
            return False, "No videos available right now.", "no_videos"
        delivery_queue.put((user_id, unsent, notify_delivery_success, notify_delivery_failure, None))
        with sqlite3.connect(DATABASE) as conn:
            conn.execute(
                'UPDATE pending_deliveries SET delivered = 1, delivered_at = ? WHERE token = ?',
                (int(time.time()), token)
            )
            conn.commit()
        return True, f"Delivering {len(unsent)} videos...", "delivered"
    except Exception as exc:
        print(f"_deliver_pending_token failed: {exc}")
        return False, "Internal error during delivery.", "error"


@app.route('/api/v2/deliver', methods=['POST'])
def api_v2_deliver():
    from flask import request, jsonify
    data = request.get_json(silent=True) or {}
    if data.get('secret') != DELIVERY_API_SECRET:
    ...
    if not token or not user_id or not count:
    ...
    try:
        with sqlite3.connect(DATABASE) as conn:
            conn.execute(
                ...
                'VALUES (?, ?, ?, ?, ?, ?)',
                (token, int(user_id), int(count), int(stars), charge_id, int(time.time()))
            )
            conn.commit()
    except Exception as exc:
        print(f"api_v2_deliver insert failed: {exc}")
    ok, msg, _code = _deliver_pending_token(token)
    if ok:
        try:
            with sqlite3.connect(DATABASE) as conn:
                conn.execute(
                    'INSERT OR IGNORE INTO payments (user_id, payment_id, amount, currency) VALUES (?, ?, ?, ?)',
                    (int(user_id), charge_id or f"v2_{token}", int(stars), 'XTR')
                )
                conn.commit()
            update_user_milestone(int(user_id), int(stars))
        except Exception as exc:
            print(f"api_v2_deliver payments record failed: {exc}")
        for admin_id in NOTIFY_IDS:
            try:
                bot.send_message(
                    admin_id,
                    f"{E_STAR} <b>New Purchase (via @{PAYMENT_BOT_USERNAME})!</b>\n\n"
                    f"👤 User ID: <code>{user_id}</code>\n"
                    f"💰 Amount: {stars} XTR\n"
                    f"📦 Package: {count} Videos",
                    parse_mode='HTML'
                )
            except Exception:
                pass
...
def start_keyboard(user_id=None):
    ...
    keyboard.add(
        styled_button(text=get_string('buy_5', lang), url=f"https://t.me/{PAYMENT_BOT_USERNAME}?start=buy_5", style="primary", emoji_id=star_emoji_id),
        styled_button(text=get_string('buy_50', lang), url=f"https://t.me/{PAYMENT_BOT_USERNAME}?start=buy_50", style="primary", emoji_id=star_emoji_id)
    )
    keyboard.add(
        styled_button(text="⭐ 100 Stars ➔ 111 Videos", url=f"https://t.me/{PAYMENT_BOT_USERNAME}?start=buy_111", style="success", emoji_id=star_emoji_id)
    )
...
def handle_offer_menu(call):
    ...
    pay_url = lambda p: f"https://t.me/{PAYMENT_BOT_USERNAME}?start={p}"
    keyboard = types.InlineKeyboardMarkup(row_width=1)
    keyboard.add(
        styled_button("⭐ 100 Stars ➔ 111 Videos", url=pay_url("buy_111"), emoji_id=star_id),
        styled_button("⭐ 100 Stars ➔ 120 Videos", url=pay_url("buy_120"), emoji_id=star_id),
        styled_button("⭐ 250 Stars ➔ 350 Videos", url=pay_url("buy_350"), emoji_id=star_id),
        styled_button("⭐ 500 Stars ➔ 750 Videos", url=pay_url("buy_750"), emoji_id=star_id),
        styled_button("⭐ 1000 Stars ➔ 1600 Videos", url=pay_url("buy_1600"), emoji_id=star_id),
...
def handle_start(message):
    ...
    # v2: handle deliver token from payment bot (priority over other /start flows)
    args_early = message.text.split()
    if len(args_early) > 1 and args_early[1].startswith("deliver_"):
        token = args_early[1].replace("deliver_", "", 1)
        save_user(user_id, username)
        ok, msg, code = _deliver_pending_token(token, chat_id=message.chat.id)
        if ok:
            bot.send_message(
                message.chat.id,
                f"✅ <b>Payment verified!</b>\n\n{msg}\n\nYour videos are on the way 🎬",
                parse_mode='HTML'
            )
        elif code == 'already':
            # Already delivered via HTTP handoff — silently show main menu
            try:
                lang = get_user_language(user_id)
                bot.send_message(
                    message.chat.id,
                    ...
                    parse_mode='HTML',
                    reply_markup=start_keyboard(user_id=user_id)
                )
            except Exception as _exc:
                print(f"silent menu fallback failed: {_exc}")
        elif code == 'no_videos':
            try:
                bot.send_message(message.chat.id, get_string('no_videos', get_user_language(user_id)))
            except Exception:
                pass
        return
